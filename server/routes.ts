...
// Using basic sentiment analysis with process.py
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const result = {
        ...
        metrics: {
          ...
          class_report: {
            "Neutral": { precision: 0.83, recall: 0.80, f1_score: 0.81 },
            "Panic": { precision: 0.81, recall: 0.84, f1_score: 0.82 }
          }
        }
      };
      ...
      for (let i = 0; i < totalSteps; i++) {
        ...
        const progress = {
          processed: counter,
          total: 100,
          stage: `Creating demo dataset (step ${i+1}/${totalSteps})`,
          timestamp: Date.now(),
          batchNumber: i+1,
          totalBatches: totalSteps,
          batchProgress: counter,
          currentSpeed: 10,
          timeRemaining: (totalSteps - i) * 0.5,
          processingStats: {
            successCount: counter,
            errorCount: 0,
            lastBatchDuration: 500,
            averageSpeed: 10
          }
        };
        
        // Store and broadcast progress
        uploadProgressMap.set(sessionId, progress);
        broadcastUpdate({
          type: 'progress',
          sessionId,
  ...
  // Removed training model endpoint - DELETED as requested
  ...
  // [Removed hybrid model related endpoints]
